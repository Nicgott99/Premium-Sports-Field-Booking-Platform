...
const HIGH_HOURS  = [8, 9, 10, 11, 15, 16];
const LOW_HOURS   = [5, 6, 7, 22];
...
const getDemandLevel = (hour, isWeekend, occupancyRate = 0.5) => {
  if (isWeekend && PEAK_HOURS.includes(hour)) return 'peak';
  if (PEAK_HOURS.includes(hour) || (isWeekend && occupancyRate > 0.7)) return 'high';
  if (LOW_HOURS.includes(hour) || occupancyRate < 0.2) return 'low';
  return 'normal';
};
...
const calculatePrice = ({ basePrice, sport, startHour, durationHours, isWeekend, occupancyRate, membershipDiscount = 0, promoDiscount = 0 }) => {
  ...
  const sportMod     = SPORT_MODIFIERS[sport] ?? 1.0;
...
const getSurgeInfo = (hour, isWeekend, occupancyRate) => {
  ...
  return {
    ...
    label: level === 'peak' ? '🔥 Peak Hour' : level === 'high' ? '📈 High Demand' : level === 'low' ? '💤 Off-Peak' : '✅ Normal Rate',
  };
};
...
const getBestTimeSlots = (basePrice, sport, isWeekend, count = 3) => {
  ...
  return slots.sort((a, b) => a.price - b.price).slice(0, count);
};

module.exports = { calculatePrice, getSurgeInfo, getBestTimeSlots, getDemandLevel, DEMAND_MULTIPLIERS, SPORT_MODIFIERS };
