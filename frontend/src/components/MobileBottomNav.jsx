...
function NavItem({ item, active }) {
  return (
    ...
    >
      {item.label === 'Book' ? (
        <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#c3f400', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 20px rgba(195,244,0,0.4)', marginTop: '-1.4rem' }}>
          <span className="material-symbols-outlined" style={{ fontSize: '1.4rem', color: '#051424', fontVariationSettings: "'FILL' 1" }}>{item.icon}</span>
        </div>
      ) : (
        <span className="material-symbols-outlined" style={{ fontSize: '1.4rem', color: active ? '#c3f400' : '#506070', fontVariationSettings: active ? "'FILL' 1" : "'FILL' 0", transition: 'color 0.15s' }}>{item.icon}</span>
      )}
      <span style={{ fontSize: '0.65rem', color: item.label === 'Book' ? '#c3f400' : active ? '#c3f400' : '#506070', fontWeight: active || item.label === 'Book' ? 700 : 500, fontFamily: "'Inter',sans-serif" }}>{item.label}</span>
    </NavLink>
  );
}
NavItem.propTypes = {
