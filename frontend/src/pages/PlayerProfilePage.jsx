...
const LEVEL_COLOR = { Rookie:'#506070', Amateur:'#8ba3be', Semi:'#ff5e07', Pro:'#c3f400', Elite:'#fff' };
...
function StatBadge({ label, value }) {
  return (
    <article style={{ background:'rgba(13,28,45,0.7)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'12px', padding:'1.1rem 1.25rem', textAlign:'center' }}>
      <div style={{ fontFamily:"'Anybody',sans-serif", fontWeight:900, fontSize:'1.9rem', color:'#c3f400', lineHeight:1 }}>{value}</div>
  ...
  if (loading) {
    return (
      <div style={{ minHeight:'100vh', background:'#051424', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ width:'40px', height:'40px', border:'3px solid rgba(195,244,0,0.2)', borderTop:'3px solid #c3f400', borderRadius:'50%', animation:'spin 0.8s linear infinite' }} />
    ...
    <div style={{ minHeight:'100vh', background:'#051424', paddingTop:'5.5rem', paddingBottom:'4rem' }}>
      ...
      <div style={{ maxWidth:'960px', margin:'0 auto', padding:'0 1.25rem' }}>
        ...
        <div style={{ background:'rgba(13,28,45,0.72)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'2.5rem', marginBottom:'1.75rem', backdropFilter:'blur(16px)', position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', top:'-60px', right:'-60px', width:'220px', height:'220px', background:'radial-gradient(circle,rgba(195,244,0,0.07) 0%,transparent 70%)', pointerEvents:'none' }} />
          <div style={{ display:'flex', gap:'2rem', alignItems:'flex-start', flexWrap:'wrap' }}>
            <div style={{ width:'88px', height:'88px', borderRadius:'50%', background:'linear-gradient(135deg,rgba(195,244,0,0.15),rgba(255,94,7,0.1))', border:'3px solid rgba(195,244,0,0.35)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'2.2rem' }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', flexWrap:'wrap', marginBottom:'0.35rem' }}>
                ...
                <span style={{ background:'rgba(195,244,0,0.12)', border:'1px solid rgba(195,244,0,0.3)', color: LEVEL_COLOR[p.level] ?? '#c3f400', padding:'0.2rem 0.7rem', borderRadius:'999px', fontSize:'0.78rem', fontWeight:800, fontFamily:"'JetBrains Mono',monospace" }}>{p.level}</span>
              </div>
              ...
              <div style={{ marginBottom:'0.5rem' }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.3rem' }}>
                  ...
                  <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'0.72rem', color:'#c3f400' }}>{p.xp.toLocaleString()} / {p.nextXp.toLocaleString()}</span>
                </div>
                <div style={{ height:'6px', background:'rgba(255,255,255,0.06)', borderRadius:'999px', overflow:'hidden' }}>
                  <div style={{ height:'100%', width:`${xpPct}%`, background:'linear-gradient(90deg,#c3f400,#ff5e07)', borderRadius:'999px', transition:'width 1s ease' }} />
                </div>
              </div>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:'0.4rem', marginTop:'0.75rem' }}>
                {(p.badges ?? []).map(b => (
                  <span key={b} style={{ background:'rgba(195,244,0,0.07)', border:'1px solid rgba(195,244,0,0.18)', color:'#8ba3be', padding:'0.2rem 0.6rem', borderRadius:'999px', fontSize:'0.75rem' }}>{b}</span>
                ))}
              </div>
            </div>
            ...
            <div style={{ textAlign:'right' }}>
              ...
              <div style={{ fontFamily:"'Anybody',sans-serif", fontWeight:900, fontSize:'2.8rem', color:'#c3f400', lineHeight:1 }}>#{p.rank}</div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.65rem' }}>
            {(p.recentBookings ?? []).map((b, i) => (
              <article key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.85rem 1rem', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'10px' }}>
                ...
                <span style={{ padding:'0.2rem 0.65rem', borderRadius:'999px', fontSize:'0.75rem', fontWeight:700, background: b.result === 'Win' ? 'rgba(195,244,0,0.12)' : 'rgba(255,94,7,0.12)', color: b.result === 'Win' ? '#c3f400' : '#ff5e07', border:`1px solid ${b.result === 'Win' ? 'rgba(195,244,0,0.25)' : 'rgba(255,94,7,0.25)'}` }}>{b.result}</span>
              </article>
            ))}
          </div>
          <button onClick={() => navigate('/bookings')} style={{ marginTop:'1.25rem', width:'100%', padding:'0.7rem', background:'rgba(195,244,0,0.06)', border:'1px solid rgba(195,244,0,0.15)', borderRadius:'10px', color:'#c3f400', fontWeight:700, fontSize:'0.88rem', cursor:'pointer', fontFamily:"'Inter',sans-serif" }}>
            View All Bookings
          </button>
        </div>
      </div>
