  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'auto 1fr auto auto', gap:'1rem', alignItems:'center', padding:'1rem 1.25rem', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'12px' }}>
      ...
      <div style={{ textAlign:'right' }}>
        <div style={{ fontFamily:"'Anybody',sans-serif", fontWeight:800, fontSize:'1rem', color:'#FBBF24' }}>৳{res.amount.toLocaleString()}</div>
      ...
      <div style={{ display:'flex', gap:'0.4rem' }}>
        {res.status === 'pending' && (
          <button type="button" onClick={() => onAction(res, 'confirm')}
            style={{ background:'rgba(251,191,36,0.1)', border:'1px solid rgba(251,191,36,0.25)', color:'#FBBF24', borderRadius:'8px', padding:'0.3rem 0.65rem', fontSize:'0.75rem', fontWeight:700, cursor:'pointer' }}>Confirm</button>
        )}
    ...
    <div style={{ minHeight:'100vh', background:'#051424', paddingTop:'5.5rem', paddingBottom:'4rem' }}>
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'0 1.25rem' }}>
        ...
        <div style={{ marginBottom:'2rem' }}>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'0.72rem', color:'#FBBF24', letterSpacing:'0.12em', textTransform:'uppercase' }}>Manage Bookings</span>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:'0.85rem', marginBottom:'1.75rem' }}>
          {[['Total','event_note', reservations.length, '#f0f6ff'],['Revenue','payments',`৳${totalRevenue.toLocaleString()}`,'#FBBF24'],['Pending','pending_actions',counts.Pending,'#ff5e07'],['Confirmed','check_circle',counts.Confirmed,'#7dd3fc']].map(([label, icon, val, color]) => (
          ...
          <div style={{ display:'flex', gap:'0.35rem', flexWrap:'wrap' }}>
            {STATUS_TABS.map(t => (
              <button key={t} onClick={() => setTab(t)}
                style={{ padding:'0.38rem 0.85rem', borderRadius:'999px', border:'1px solid', fontWeight:700, fontSize:'0.78rem', cursor:'pointer', background: tab === t ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.04)', color: tab === t ? '#FBBF24' : '#506070', borderColor: tab === t ? 'rgba(251,191,36,0.35)' : 'rgba(255,255,255,0.08)', transition:'all 0.15s' }}>
