...
const STATUS_STYLE = {
  completed:{ color:'#c3f400', bg:'rgba(195,244,0,0.1)', border:'rgba(195,244,0,0.2)' },
...
function TxRow({ tx }) {
  ...
  return (
    <article style={{ display:'flex', alignItems:'center', gap:'1rem', padding:'0.9rem 1rem', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'12px' }}>
      <div style={{ width:'36px', height:'36px', borderRadius:'50%', background: isCredit ? 'rgba(195,244,0,0.1)' : 'rgba(255,94,7,0.1)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
        <span className="material-symbols-outlined" style={{ fontSize:'1rem', color: isCredit ? '#c3f400' : '#ff5e07' }}>{isCredit ? 'add_circle' : 'remove_circle'}</span>
      </div>
      <div style={{ flex:1 }}>
      ...
      <div style={{ textAlign:'right' }}>
        <div style={{ fontFamily:"'Anybody',sans-serif", fontWeight:800, fontSize:'1rem', color: isCredit ? '#c3f400' : '#ff5e07' }}>
    ...
    <div style={{ minHeight:'100vh', background:'#051424', paddingTop:'5.5rem', paddingBottom:'4rem' }}>
      <div style={{ maxWidth:'900px', margin:'0 auto', padding:'0 1.25rem' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'2.5rem' }}>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:'0.72rem', color:'#c3f400', letterSpacing:'0.12em', textTransform:'uppercase' }}>Payments</span>
        ...
        {/* Balance card */}
        <div style={{ background:'linear-gradient(135deg,rgba(13,28,45,0.9) 0%,rgba(18,33,49,0.95) 100%)', border:'1px solid rgba(195,244,0,0.18)', borderRadius:'20px', padding:'2rem 2.25rem', marginBottom:'1.5rem', backdropFilter:'blur(16px)', position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', top:'-40px', right:'-40px', width:'160px', height:'160px', borderRadius:'50%', background:'radial-gradient(circle,rgba(195,244,0,0.08) 0%,transparent 70%)', pointerEvents:'none' }} />
          ...
          <div style={{ fontFamily:"'Anybody',sans-serif", fontWeight:900, fontSize:'3.5rem', color:'#c3f400', letterSpacing:'-0.02em', lineHeight:1.1, margin:'0.25rem 0 0.75rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.25rem', marginBottom:'1.5rem' }}>
          ...
          <div style={{ background:'rgba(13,28,45,0.72)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', padding:'1.5rem', backdropFilter:'blur(14px)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginBottom:'0.85rem' }}>
              {TOPUP_OPTIONS.map(a => (
                <button key={a} onClick={() => { setTopupAmt(a); setCustom(''); }}
                  style={{ padding:'0.55rem', borderRadius:'10px', border:'1px solid', fontWeight:700, fontSize:'0.82rem', cursor:'pointer', background: topupAmt === a && !custom ? 'rgba(195,244,0,0.15)' : 'rgba(255,255,255,0.03)', color: topupAmt === a && !custom ? '#c3f400' : '#506070', borderColor: topupAmt === a && !custom ? 'rgba(195,244,0,0.35)' : 'rgba(255,255,255,0.08)', transition:'all 0.15s' }}>
                  ৳{a.toLocaleString()}
                </button>
              ))}
            </div>
            ...
            <button style={{ width:'100%', padding:'0.65rem', background:'#c3f400', border:'none', borderRadius:'10px', color:'#0a1200', fontFamily:"'Anybody',sans-serif", fontWeight:800, fontSize:'0.9rem', cursor:'pointer' }}>
          ...
          <div style={{ background:'rgba(13,28,45,0.72)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', padding:'1.5rem', backdropFilter:'blur(14px)', display:'flex', flexDirection:'column', gap:'0.85rem' }}>
            ...
            {[['payments','Total Top-ups',`৳${w.totalTopUp.toLocaleString()}`],['sports','Bookings Paid',w.transactions.filter(t=>t.type==='debit').length],['redeem','Cashback Earned',`৳${w.transactions.filter(t=>t.desc.includes('cashback')).reduce((s,t)=>s+t.amount,0)}`]].map(([icon, label, val]) => (
              <div key={label} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.75rem', background:'rgba(255,255,255,0.03)', borderRadius:'10px' }}>
                ...
                <span style={{ color:'#c3f400', fontWeight:700, fontSize:'0.9rem' }}>{val}</span>
              </div>
            ))}
          </div>
        </div>
